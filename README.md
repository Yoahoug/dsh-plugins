...
| [`@dsh-plugins/session-archive-restore`](packages/session-archive-restore/README.zh.md) | webServer + storageDomain + sessionPersistence | Consumer | 为 dsh-launcher 提供本地归档会话热恢复与永久删除接口;删除同步清理工作区引用、projection cache 和 JSONL 会话日志 |
...
- [docs/session-archive-restore-development.md](docs/session-archive-restore-development.md) — session-archive-restore 归档会话恢复、永久删除设计与部署记录

/**
 * The plugin's settings section layered over the composition entry. The section
 * deliberately registers under the `web-search-deepseek` namespace — the one
 * the Web settings card and the apiproxy allow-list hardcode (Option A) — so
 * the assertions here pin that reuse, not a tavily-owned namespace.
 */
...
import { WEB_SEARCH_TAVILY_SETTINGS_NAMESPACE } from '@dsh-plugins/web-search-tavily'
...
const ONE_RESULT = {
  results: [{ url: 'https://a.test', title: 'A', content: 'snippet' }],
}
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(tavilyPlugin, { apiKey: 'tv-key', baseURL: 'https://search.entry.test/v1' })
...
describe('web-search-tavily settings section', () => {
  ...
  it('keeps the literal key out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_TAVILY_SETTINGS_NAMESPACE, { apiKey: 'tv-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-deepseek')

    expect(JSON.stringify(descriptor)).not.toContain('tv-stored-secret')
